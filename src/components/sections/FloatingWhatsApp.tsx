...
interface FloatingWhatsAppProps { config: SiteConfig }
...
export default function FloatingWhatsApp({ config }: FloatingWhatsAppProps) {
  ...
  if (business.whatsapp === 'PENDIENTE_DE_CONFIRMAR') return null
  ...
  return (
    <a
      ...
      className={`whatsapp-float transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
    >
      ...
      <span className="label font-body text-sm font-semibold">{business.whatsappMessage.split('.')[0]}</span>
    </a>
  )
}
